...
This file is the actual thesis of the project. Every variable lives in one of three states — `Owned`, `Moved`, or `PartiallyMoved(fields)` — tracked in a `HashMap<String, VarState>` that gets threaded through the whole AST walk.
    ...
    match expr {
        Expr::Ident(name) => match scope.get(name) {
            ...
            Some(VarState::PartiallyMoved(fields)) => {
                Err(format!("use of partially moved value `{}`", name))
            }
            None => Err(format!("use of undeclared variable `{}`", name)),
        },
        // ... Field, Call, StructLit recurse into their sub-expressions
    }
}
```
...
Every simplification above is Warden skipping *one specific piece* of what real Rust does. Laid out plainly:
